...
// Example usage functions for your HTTP tool:
...
// Example of how to use this in your HTTP tool:
//
// 1. Merge TOML files:
//    jsonData, err := MergeTomlFiles("method.toml", "headers.toml", "body.toml")
//
// 2. Update a header:
//    err := UpdateTomlValue("headers.toml", "Authorization", "Bearer new-token")
//
// 3. Get a specific value:
//    handler, _ := NewTomlHandler("method.toml")
//    url := handler.GetAsString("url")
//    method := handler.GetAsString("method")
//
// 4. Manual merging with control:
//    method, _ := NewTomlHandler("method.toml")
//    headers, _ := NewTomlHandler("headers.toml") 
//    body, _ := NewTomlHandler("body.toml")
//    method.MergeMultiple(headers, body)
//    jsonPayload, _ := method.ToJSON()